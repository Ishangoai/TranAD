...
if __name__ == '__main__':
	...
	loss, y_pred = backprop(0, model, testD, testO, optimizer, scheduler, training=False)#normaly testD,testO

	### Plot curves
	if not args.test:
		if 'TranAD' in model.name: testO = torch.roll(trainO, 1, 0) #normaly testO
		plotter2(f'{args.model}_{args.dataset}', testO, y_pred, loss)#normaly testO

	""" ### Scores
	df = pd.DataFrame()
	lossT, _ = backprop(0, model, trainD, trainO, optimizer, scheduler, training=False)
	for i in range(loss.shape[1]):
		lt, ls = lossT[:, i], loss[:, i]
